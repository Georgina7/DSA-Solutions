...
# All the arrays are 0-indexed. Return a list of boolean elements answer, where answer[i] is true if the subarray nums[l[i]], nums[l[i]+1], ... , nums[r[i]] 
...
class Solution:
    def checkArithmeticSubarrays(self, nums: List[int], l: List[int], r: List[int]) -> List[bool]:
        ...
        for i in range(len(l)):
            ...
            sub_arr.sort()
            d = sub_arr[0] - sub_arr[1]
            for j in range(len(sub_arr) - 1):
                if sub_arr[j] - sub_arr[j + 1] != d:
                    ans[i] = False
        return ans
